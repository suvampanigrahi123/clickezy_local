...
import { setError, setStudio } from '../../../redux/slices/bookingSlice';
...
export const BookingStudio = ({ title, studio_id }) => {
  return (
    <>
      <label className="flex flex-col justify-start items-start self-stretch flex-grow-0 flex-shrink-0 relative gap-2">
        ...
        <StudioList studio_id={studio_id} />
      </label>
    </>
  );
};

function StudioList({ studio_id }) {
  const dispatch = useDispatch();
  const [selected, setSelected] = useState({
    studio_name: 'Select',
    studio_id: 0,
  });
  const { bookingFor, bookingLocation } = useSelector((state) => state.booking);
  ...
  const { data: studioList, isLoading } = useSwr(
    ...
    () => api.getStudioById(dependent)
  );
  const studios = studioList?.data?.data;
  useEffect(() => {
    setItems([]);
    setSelected({ studio_name: 'Select', id: 0 });
    dispatch(setStudio({ studio_name: 'Select', id: 0 }));
    if (bookingFor.id && bookingLocation.id) {
      setDependent({
        locationId: bookingLocation.id,
        categoryId: bookingFor.id,
      });
    }
  ...
  }, [bookingFor, bookingLocation, studios, studio_id]);
                              ...
                              <div className="flex justify-between items-center flex-grow relative gap-4">
                                ...
                                <div className="flex justify-start items-center flex-grow-0 flex-shrink-0 relative gap-1 pr-3">
                                  ...
                                  <p className="flex-grow-0 flex-shrink-0 text-left capitalize text-[#2fa3ff]">
                                    {Math.round(
                                      studio?.details &&
                                        studio?.details[0]?.price_details
                                          ?.hour_price
                                    ) || '-'}
                                    {' / hr'}
                                  </p>
